...
{
    public class LimepayApiParcelsResource : LimepayApiAbstractResource
    {
        public LimepayApiParcelsResource(LimepayApi client) : base(client)
        {
            Resource = "parcels";
        ...
        {
            return await Client.Create<ParcelCancel>($"{HostUrl}{Resource}/{parcelId}/cancel", string.Empty, string.Empty, DateTimeFormat);
        }

        public async Task<string> GetReturnPortalUrl(int parcelId)
        {
            return await Client.Get<string>($"{HostUrl}{Resource}/{parcelId}/return_portal_url", null, "url", DateTimeFormat);
        }
    }
}
